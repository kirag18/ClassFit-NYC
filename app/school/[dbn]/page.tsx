...
export default async function SchoolDetailPage({
  ...
  return (
    <div className="mx-auto max-w-4xl w-full px-6 py-6 space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        ...
        <div className="space-y-4">
          {bands.map((b) => {
            ...
            return (
              <div key={b.grade_band} className="border-b border-slate-100 last:border-0 pb-4 last:pb-0">
                ...
                {b.data_quality === "suspect" && (
                  <div className="mb-1.5 text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded px-2 py-1">
                ...
                <p className="text-sm text-slate-600">{plainLanguageStatus(b.avg_class_size, b.target_cap)}</p>
              </div>
            );
          })}
          {bands.length === 0 && <p className="text-sm text-slate-500">No class size data on file.</p>}
        </div>
      </section>

      {/* Building utilization */}
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <h2 className="text-base font-semibold text-slate-900 mb-3">Building utilization</h2>
        {building ? (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            <div>
              <div className="text-slate-500">Utilization</div>
              <div className="text-lg font-semibold text-slate-900">{building.utilization_pct}%</div>
            </div>
            <div>
              <div className="text-slate-500">
                ...
                {!!building.co_located && <span className="text-slate-400"> (shared)</span>}
              </div>
              <div className="text-lg font-semibold text-slate-900">{building.enrollment}</div>
            </div>
            <div>
              <div className="text-slate-500">Target capacity</div>
              <div className="text-lg font-semibold text-slate-900">{building.capacity}</div>
            </div>
            <div>
              <div className="text-slate-500">Co-located</div>
          ...
          <p className="text-sm text-slate-500 mb-2">No Blue Book utilization data on file for this building.</p>
        )}

        {/* School-specific stats, as opposed to the building-level figures above. */}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm mt-4 pt-4 border-t border-slate-100">
          <div>
            <div className="text-slate-500">This school&apos;s enrollment</div>
            <div className="text-lg font-semibold text-slate-900">
              {school.enrollment != null ? school.enrollment.toLocaleString() : "—"}
            </div>
            {school.enrollment == null && (
              <div className="text-xs text-slate-400">not in the Blue Book</div>
            )}
          </div>
          <div>
            <div className="text-slate-500">Standard classrooms</div>
            <div className="text-lg font-semibold text-slate-900">
              {standardRoomCount > 0 ? standardRoomCount.toLocaleString() : "—"}
            </div>
            {standardRoomCount === 0 && <div className="text-xs text-slate-400">no room data</div>}
          </div>
          <div>
            <div className="text-slate-500">Cafeteria size</div>
            <div className="text-lg font-semibold text-slate-900">
              {cafeteriaRoom?.sqft ? `${cafeteriaRoom.sqft.toLocaleString()} sqft` : "—"}
            </div>
            {!cafeteriaRoom?.sqft && <div className="text-xs text-slate-400">no cafeteria on record</div>}
          </div>
        </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <h2 className="text-base font-semibold text-slate-900 mb-1">Space toolkit</h2>
